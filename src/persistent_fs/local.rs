...
impl PersistentFSBackend for LocalFSBackend {
    ...
    async fn download_url(self, local_host: &str) -> std::io::Result<String> {
        ...
        use crate::{auth::{encode_jwt, generate_file_download_claims, generate_send_claims}, CONFIG};
        ...
        if path.starts_with(&*ATTACHMENTS_PREFIX) {
            ...
            let token = encode_jwt(&generate_file_download_claims(cipher_uuid.to_string(), attachment_id.to_string()));
            ...
            let token = encode_jwt(&generate_send_claims(send_id, file_id));
