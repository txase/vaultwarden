...
async fn get_cached_icon(path: &str) -> Option<Vec<u8>> {
    ...
    if let Ok(operator) = CONFIG.opendal_operator_for_path_type(PathType::IconCache).await {
...
async fn file_is_expired(path: &str, ttl: u64) -> Result<bool, Error> {
    let operator = CONFIG.opendal_operator_for_path_type(PathType::IconCache).await?;
    let meta = operator.stat(path).await?;
    let modified = meta.last_modified()
        .ok_or_else(|| std::io::Error::other(format!("No last modified time for `{path}`")))?;
...
async fn icon_is_negcached(path: &str) -> bool {
    ...
    match expired {
        ...
        Ok(true) => {
            match CONFIG.opendal_operator_for_path_type(PathType::IconCache).await {
                Ok(operator) => if let Err(e) = operator.delete_iter([miss_indicator]).await {
...
async fn save_icon(path: &str, icon: Vec<u8>) {
    let operator = match CONFIG.opendal_operator_for_path_type(PathType::IconCache).await {
