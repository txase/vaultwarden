...
pub async fn initialize_keys() -> Result<(), Error> {
    async fn read_key(create_if_missing: bool) -> Result<(Rsa<openssl::pkey::Private>, Vec<u8>), std::io::Error> {
        ...
        let operator = CONFIG.opendal_operator_for_path_type(PathType::RsaKey).await
        ...
        let priv_key_buffer = match operator.read(&rsa_key_filename).await {
            ...
            Err(e) if e.kind() == opendal::ErrorKind::NotFound => return Err(Error::new(ErrorKind::NotFound, "Private key not found")),
        ...
        if let Some(priv_key_buffer) = priv_key_buffer {
            ...
            Err(Error::new(ErrorKind::NotFound, "Private key created, forcing attempt to read it again").into())
        }
    }
