...
impl Attachment {
    ...
    pub async fn get_url(&self, host: &str) -> Result<String, crate::Error> {
        let operator = CONFIG.opendal_operator_for_path_type(PathType::Attachments).await?;
        ...
        if operator.info().scheme() == opendal::Scheme::Fs {
            ...
            Ok(operator.presign_read(&self.get_file_path(), Duration::from_secs(5 * 60)).await
