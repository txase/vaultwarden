...
static CONFIG_FILENAME: LazyLock<String> = LazyLock::new(|| {
    ...
    path
        .file_name()
        .unwrap_or(&std::ffi::OsStr::new("config.json"))
        .to_str()
    ...
    ($(
        ...
        impl ConfigBuilder {
            ...
            async fn from_file() -> Result<Self, Error> {
                let operator = opendal_operator_for_path(&CONFIG_FILE_PARENT_DIR).await?;
...
async fn opendal_operator_for_path(path: &str) -> Result<opendal::Operator, Error> {
    // Cache of previously built operators by path
    static OPERATORS_BY_PATH: LazyLock<Mutex<HashMap<String, opendal::Operator>>> = LazyLock::new(|| Mutex::new(HashMap::new()));
    
    let mut operators_by_path = OPERATORS_BY_PATH.lock().map_err(|e| format!("Failed to lock OpenDAL operators cache: {e}"))?;
    ...
    let operator = if path.starts_with("s3://") {
        #[cfg(not(s3))]
        return Err("S3 support is not enabled".into());
...
fn opendal_s3_operator_for_path(path: &str) -> Result<opendal::Operator, Error> {
    ...
    impl reqsign::AwsCredentialLoad for OpenDALS3CredentialLoader {
        async fn load_credential(&self, _client: reqwest::Client) -> anyhow::Result<Option<reqsign::AwsCredential>> {
            ...
            static DEFAULT_CREDENTIAL_CHAIN: OnceCell<aws_config::default_provider::credentials::DefaultCredentialsChain> = OnceCell::const_new();
    ...
    let url = Url::parse(path)
        .map_err(|e| format!("Invalid path S3 URL path {path:?}: {e}"))?;

    let bucket = url.host_str()
        .ok_or_else(|| format!("Missing Bucket name in data folder S3 URL {path:?}"))?;
...
impl Config {
    ...
    pub async fn opendal_operator_for_path_type(&self, path_type: PathType) -> Result<opendal::Operator, Error> {
        ...
        opendal_operator_for_path(&path).await
    }
